...
class SQLTool:
    ...
    def _generate_sql(self, question: str, syntax_error: str = None) -> str:
        ...
        system_prompt = (
            ...
            "- **Business Metric Calculation Rules**:\n"
            "  * **Revenue (Sales Amount)**: Calculate as the sum of total_amount (e.g., `SUM(s.total_amount)`). Do not calculate from unit_price * quantity manually, and do not use catalog price `p.price`.\n"
            "  * **Cost of Goods Sold (COGS)**: Calculate as quantity * cost (e.g., `SUM(s.quantity * p.cost)`). Requires JOINing the `products` table.\n"
            "  * **Profit**: Calculate as Revenue minus Cost of Goods Sold (e.g., `SUM(s.total_amount - s.quantity * p.cost)`). Always subtract the product production cost from the actual revenue received. Never omit product cost when profit is requested.\n"
